...
void OrderedDictionary::add(IKey *k, ICollectible *val)
{
    ...
    {
        parentComparison = key->compare(current->getKey());
        if(parentComparison == EQUAL){  // la clave ya está, se reemplaza
            current->setVal(val);
...
void OrderedDictionary::remove(IKey *k)
{
    ...
    {
        parentComparison = key->compare(current->getKey());
        if(parentComparison == EQUAL)  // la clave ya está, se puede borrar
            break;
